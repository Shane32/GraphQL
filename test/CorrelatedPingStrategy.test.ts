...
describe("CorrelatedPingStrategy", () => {
  ...
  describe("real timer integration", () => {
    ...
    it("should work with real timers for ack timeout", (done) => {
      const shortTimeoutStrategy = new CorrelatedPingStrategy(50, 1000, 100);
      shortTimeoutStrategy.attach(mockApi);
      shortTimeoutStrategy.onOpen?.();
    ...
    it("should work with real timers for ping/pong", (done) => {
      const shortTimeoutStrategy = new CorrelatedPingStrategy(50, 100, 50);
      shortTimeoutStrategy.attach(mockApi);
      shortTimeoutStrategy.onOpen?.();
      shortTimeoutStrategy.onAck?.();

      // Should send ping after 100ms
      setTimeout(() => {
        expect(mockApi.send).toHaveBeenCalledWith({
          type: "ping",
          payload: { id: expect.any(String) },
        ...
        // Should timeout after another 50ms without pong
        setTimeout(() => {
          expect(mockApi.abort).toHaveBeenCalledWith(CloseReason.Timeout);
          done();
        }, 75);
    ...
    it("should handle pong correctly with real timers", (done) => {
      const shortTimeoutStrategy = new CorrelatedPingStrategy(50, 100, 200);
      shortTimeoutStrategy.attach(mockApi);
      shortTimeoutStrategy.onOpen?.();
      shortTimeoutStrategy.onAck?.();
